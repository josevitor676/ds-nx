...
export const Tooltip = React.forwardRef<HTMLDivElement, TooltipProps>(
  (
    { title, description, arrowPosition = "default", children, delayDuration = 200, className },
    _ref
  ) => (
    <RadixTooltip.Root delayDuration={delayDuration}>
      ...
      <RadixTooltip.Portal>
        ...
        >
          {title && <p className={tooltipTitleVariants()}>{title}</p>}
          <p className={tooltipDescriptionVariants()}>{description}</p>

          <RadixTooltip.Arrow className="ds-fill-neutral-50" width={10} height={5} />
        </RadixTooltip.Content>
      </RadixTooltip.Portal>
    </RadixTooltip.Root>
  )
