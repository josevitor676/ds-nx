...
export function buildCSSVariables(tokens: ThemeTokens): Record<string, string> {
  ...
  // ── Colors ────────────────────────────────────────────────────────────────
  const colorGroups = ["primary", "secondary", "neutral", "success", "warning", "error"] as const
  ...
  if (tokens.colors.sidebar) {
    for (const [key, value] of Object.entries(tokens.colors.sidebar)) {
      const cssKey = key.replace(/([A-Z])/g, (m) => `-${m.toLowerCase()}`)
      vars[`--ds-color-sidebar-${cssKey}`] = value
    }
  }

  if (tokens.colors.chart) {
    for (const [key, value] of Object.entries(tokens.colors.chart)) {
      vars[`--ds-color-chart-${key}`] = value
    }
  }
...
export function applyThemeToElement(element: HTMLElement, tokens: ThemeTokens): void {
